...
class PhotometricFisheye(InverseWarp):
    ...
    def inverse_warp(self, source_img, norm, grid_xy, car_mask,
        ...
        flow_x = (image_coords[:, 0, :] + 1) / 2 * (width - 1) - grid_xy[:, 0, :]
        flow_y = (image_coords[:, 1, :] + 1) / 2 * (height - 1) - grid_xy[:, 1, :]
        flow = torch.cat([torch.unsqueeze(flow_x, dim=1), torch.unsqueeze(flow_y, dim=1)], dim=1)\
            .reshape(batch_size, 2, height, width)
        ...
        if self.input_mask:
            ...
            projected_img = projected_img * car_hood
            mask = mask * car_hood

        return projected_img, mask, world_coords[:, 0:3, :], flow

    def compute_flow(self, norm, grid_xy, car_mask,
                     extrinsic_mat, K, D, theta_lut, angle_lut) -> tuple:
        batch_size, _, height, width = norm.size()
        norm = norm.reshape(batch_size, height * width)

        world_coords = self.img2world(norm, theta_lut, angle_lut, extrinsic_mat)  # [B x 4 x H * W]
        image_coords, mask = self.world2img(world_coords, K, D, height, width)

        image_coords[:, 0, :] = (image_coords[:, 0, :] + 1) / 2 * (width - 1) - grid_xy[:, 0, :]
        image_coords[:, 1, :] = (image_coords[:, 1, :] + 1) / 2 * (height - 1) - grid_xy[:, 1, :]

        return world_coords[:, 0:3, :], image_coords.reshape(batch_size, 2, height, width), mask

    def fisheye_inverse_warp(self, inputs, outputs):
        _, _, height, width = outputs[("norm", 0)].size()
        grid_y, grid_x = torch.meshgrid(torch.arange(height), torch.arange(width))
        grid_xy = torch.cat([grid_x.reshape(1, 1, width*height), grid_y.reshape(1, 1, width*height)], dim=1).cuda()
        for scale in range(self.num_scales):
            for frame_id in self.frame_idxs[1:]:
                ...
                #essential_mat = inputs[("cam_T_cam", -1)]


                outputs[("color", frame_id, scale)], outputs[("ego_mask", frame_id, scale)], \
                outputs[("points3D", frame_id, scale)], outputs[("flow", frame_id, scale)] = \
                    self.inverse_warp(image,
                                      ...
                                      angle_lut)
                """
                outputs[("points3D", frame_id, scale)], outputs[("flow", frame_id, scale)], outputs[("ego_mask", frame_id, scale)] = \
                    self.compute_flow(norm, grid_xy, car_mask, essential_mat, intrinsic_mat, distortion_coeffs, theta_lut, angle_lut)
                """
